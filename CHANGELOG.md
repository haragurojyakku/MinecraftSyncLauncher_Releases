...
## [0.6.4] — Mod一覧ファイルの自動生成 — 2026-08-01

### 追加 (Added)

**sync-server モジュール**
- `server/MODS.md`を自動生成するようにした(`ServerFilesService.writeModListReport`)。
  起動時、およびMod追加/削除/有効・無効切替/カテゴリ変更のたびに`mods/`を再スキャンして
  Mod名・サイズ・状態・カテゴリの一覧を書き出す。ランチャーを起動しなくても、リポジトリ/
  サーバーにアクセスできれば現在の導入状況を確認できる。
